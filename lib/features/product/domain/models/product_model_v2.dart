...
class Product {
  int? _id;                         // Maps to "id"
  String? _addedBy;                 // Maps to "added_by"
  int? _userId;                     // Maps to "user_id"
  String? _name;                    // Maps to "name"
  String? _slug;                    // Maps to "slug"
  String? _productType;             // Maps to "product_type"
  List<CategoryIds>? _categoryIds;  // Maps to "category_ids" (parse from JSON string)
  String? _unit;                    // Maps to "unit"
  List<String>? _images;            // Maps to "images" (parse from JSON string)
  List<ImageFullUrl>? _imagesFullUrl; // If you have full URLs, create a parsing logic
  String? _thumbnail;               // Maps to "thumbnail"
  ImageFullUrl? _thumbnailFullUrl;  // If you have a full URL for thumbnail, create a parsing logic
  List<ProductColors>? _colors;     // Maps to "colors" (parse from JSON string)
  List<String>? _attributes;        // Maps to "attributes" (parse from JSON string)
  List<ChoiceOptions>? _choiceOptions; // Maps to "choice_options" (parse from JSON string)
  List<Variation>? _variation;      // Maps to "variation" (parse from JSON string)
  double? _unitPrice;               // Maps to "unit_price"
  double? _purchasePrice;           // Maps to "purchase_price"
  double? _tax;                     // Maps to "tax"
  String? _taxModel;                // Maps to "tax_model"
  int? _minQty;                     // Maps to "min_qty"
  int? _refundable;                 // Maps to "refundable"
  String? _digitalProductType;      // Maps to "digital_product_type"
  String? _digitalFileReady;        // Maps to "digital_file_ready"
  String? _taxType;                 // Maps to "tax_type"
  double? _discount;                // Maps to "discount"
  String? _discountType;            // Maps to "discount_type"
  int? _currentStock;               // Maps to "current_stock"
  String? _details;                 // Maps to "details"
  String? _createdAt;               // Maps to "created_at"
  String? _updatedAt;               // Maps to "updated_at"
  List<Rating>? _rating;            // You need to parse ratings separately if available
  double? _shippingCost;            // Maps to "shipping_cost"
  int? _isMultiPly;                 // Maps to "multiply_qty"
  int? _reviewCount;                // You might need a different field in JSON for this
  String? _videoUrl;                // Maps to "video_url"
  int? _minimumOrderQty;            // Maps to "minimum_order_qty"
  int? wishList;                    // This might not have a direct match in your JSON
  Brand? brand;                     // You'll need to create a mapping if "brand" exists
  ImageFullUrl? digitalFileReadyFullUrl; // If applicable, map this based on digitalFileReady
