...
class CartBottomSheetWidgetState extends State<CartBottomSheetWidget> {
    ...
    return Column(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: Theme.of(context).highlightColor, borderRadius: const BorderRadius.only(topRight: Radius.circular(20), topLeft: Radius.circular(20))),
          child: Consumer<ProductDetailsController>(
            builder: (ctx, details, child) {
              ...
              return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                Align(alignment: Alignment.centerRight, child: InkWell(onTap: () => Navigator.pop(context), child: Padding(padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeSmall), child: Icon(Icons.cancel, color: Theme.of(context).hintColor, size: 30)))),
                          ...
                          children: [
                            Stack(children: [
                              Container(
                                  width: 100,
                                  height: 100,
                                  decoration: BoxDecoration(borderRadius: BorderRadius.circular(5), border: Border.all(width: .5, color: Theme.of(context).primaryColor.withOpacity(.20))),
                                  child: ClipRRect(
                              ...
                              widget.product!.discount! > 0
                                  ? Container(
                                      ...
                                      decoration: BoxDecoration(color: Theme.of(context).colorScheme.error, borderRadius: const BorderRadius.vertical(top: Radius.circular(Dimensions.paddingSizeExtraSmall))),
                                      child: Padding(padding: const EdgeInsets.all(5), child: Text(PriceConverter.percentageCalculation(context, widget.product!.unitPrice, widget.product!.discount, widget.product!.discountType), style: titilliumRegular.copyWith(color: const Color(0xFFFFFFFF), fontSize: Dimensions.fontSizeDefault))),
                                    )
                                  : const SizedBox(width: 93)
                            ]),
                            const SizedBox(height: 10),
                            Text('$stock ${getTranslated('in_stock', context)}', style: titilliumRegular.copyWith(fontSize: Dimensions.fontSizeLarge), maxLines: 1),
                          ],
                        ),
                        const SizedBox(width: 20),
                        Expanded(
                            child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          Text(widget.product!.name ?? '', style: titilliumRegular.copyWith(fontSize: Dimensions.fontSizeLarge), maxLines: 2, overflow: TextOverflow.ellipsis),
                          // const SizedBox(height: Dimensions.paddingSizeSmall),
                          Row(
                            children: [
                              const Icon(Icons.star_rate_rounded, color: Color(0xFFFB9C1F)),
                              Text(double.parse(ratting).toStringAsFixed(1), style: titilliumSemiBold.copyWith(fontSize: Dimensions.fontSizeLarge), maxLines: 2, overflow: TextOverflow.ellipsis),
                            ],
                          ),
                          // const SizedBox(height: Dimensions.paddingSizeSmall),
                          Row(children: [
                            ...
                            widget.product!.discount! > 0
                                ...
                                : const SizedBox(),
                            // const SizedBox(width: Dimensions.paddingSizeDefault),
                            Text(PriceConverter.convertPrice(context, widget.product!.unitPrice, discountType: widget.product!.discountType, discount: widget.product!.discount), style: titilliumRegular.copyWith(color: ColorResources.getPrimary(context), fontSize: Dimensions.fontSizeExtraLarge))
                          ])
                        ]))
                      ])
                    ])),
                                    ...
                                    child: InkWell(
                                      ...
                                      child: Container(
                                        ...
                                        decoration: BoxDecoration(
                                          ...
                                          color: details.variationIndex![index] == i ? Theme.of(context).primaryColor : Theme.of(context).highlightColor,
                                        ),
                                        child: Container(
                                          decoration: BoxDecoration(
                                            ...
                                            border: Border.all(
                                              width: 2,
                                              color: details.variationIndex![index] == i ? Theme.of(context).cardColor : const Color(0x00FFFFFF),
                                            ),
                                          ),
                                          child: Padding(
                                            padding: const EdgeInsets.symmetric(horizontal: Dimensions.paddingSizeDefault),
                                            child: Center(child: Text(widget.product!.choiceOptions![index].options![i].trim(), maxLines: 1, overflow: TextOverflow.ellipsis, style: titilliumRegular.copyWith(fontSize: Dimensions.fontSizeDefault, color: (details.variationIndex![index] != i && !Provider.of<ThemeController>(context, listen: false).darkTheme) ? Theme.of(context).primaryColor : const Color(0xFFFFFFFF)))),
                                          ),
                                        ),
                                      ),
                                    ),
                  ...
                  Spacer(),
                      Column(
                        ...
                        children: [
                          ...
                          Text(PriceConverter.convertPrice(context, priceWithQuantity), style: titilliumBold.copyWith(color: ColorResources.getPrimary(context), fontSize: Dimensions.fontSizeLarge)),
  ...
  void _buyNow(CartModelBody cart, BuildContext context, List<ChoiceOptions> choices, List<int>? variationIndexes, Response<dynamic>? response) {
    if (response!.data['status'] == 1) {
      ...
      showDialog(context: context, builder: (context) => Dialog(backgroundColor: color.Colors.transparent, child: ChooseShippingMethodDialog(shippingMethodList, cart, choices, variationIndexes, _navigateToCheckoutScreen)));
    }
  }
  ...
  void _navigateToCheckoutScreen(BuildContext context, CartModel cart, double shippingCost) {
    ...
    Navigator.of(context).push(MaterialPageRoute(
        builder: (context) => CheckoutScreen(cartList: [
              cart
            ], discount: discount, tax: tax, totalOrderAmount: amount, shippingFee: shippingAmount, quantity: totalQuantity, onlyDigital: !hasPhysical, hasPhysical: hasPhysical)));
  }
}
