    ...
    return InkWell(
          ...
          padding: widget.isHomePage
              ? EdgeInsets.only(
                  ...
                  right: widget.index + 1 == widget.length
                      ? Dimensions.paddingSizeDefault
                      : (Provider.of<LocalizationController>(context, listen: false).isLtr && widget.isHomePage)
                          ? 0
                          : Dimensions.paddingSizeDefault,
                  bottom: widget.isHomePage ? Dimensions.paddingSizeExtraSmall : Dimensions.paddingSizeDefault)
        ...
        decoration: BoxDecoration(
          ...
          color: Theme.of(context).highlightColor,
            ...
            child: Column(
              children: [
                SizedBox(height: widget.isHomePage ? 57 : 120, width: double.infinity, child: ClipRRect(borderRadius: const BorderRadius.only(topLeft: Radius.circular(Dimensions.paddingSizeSmall), topRight: Radius.circular(Dimensions.paddingSizeSmall)), child: CustomImageWidget(image: widget.sellerModel!.id == 0 ? splashController.configModel!.companyCoverImage ?? '' : '${splashController.baseUrls!.shopImageUrl}/banner/${widget.sellerModel!.shop!.banner ?? ''}'))),
                Row(children: [
                  Container(
          ...
          color: Theme.of(context).highlightColor,
                      ...
                      child: Stack(children: [
                        Container(width: 60, height: 60, decoration: BoxDecoration(borderRadius: const BorderRadius.all(Radius.circular(Dimensions.paddingSizeOverLarge)), color: Theme.of(context).highlightColor), child: ClipRRect(borderRadius: const BorderRadius.all(Radius.circular(Dimensions.paddingSizeOverLarge)), child: CustomImageWidget(image: widget.sellerModel!.id == 0 ? splashController.configModel!.companyIcon ?? '' : '${splashController.baseUrls!.shopImageUrl!}/${widget.sellerModel!.shop?.image}', width: 60, height: 60))),
                        if ((widget.sellerModel!.shop!.temporaryClose ?? false) || vacationIsOn) Container(decoration: BoxDecoration(color: Colors.black.withOpacity(.5), borderRadius: const BorderRadius.all(Radius.circular(Dimensions.paddingSizeOverLarge)))),
                        (widget.sellerModel!.shop!.temporaryClose ?? false)
                            ? Center(child: Text(getTranslated('temporary_closed', context)!, textAlign: TextAlign.center, style: textRegular.copyWith(color: Colors.white, fontSize: Dimensions.fontSizeSmall)))
                            : vacationIsOn == true
                                ? Center(
                                    child: Text(
                    ...
                    child: Row(children: [
                      Expanded(
                          child: Container(
                              ...
                              decoration: BoxDecoration(color: Theme.of(context).hintColor.withOpacity(.125), borderRadius: BorderRadius.circular(Dimensions.paddingSizeExtraSmall)),
                              child: Row(mainAxisAlignment: MainAxisAlignment.center, children: [
                                Text(
                                  NumberFormat.compact().format(widget.sellerModel?.productCount ?? 0),
